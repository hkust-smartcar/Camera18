...
void LeftEdge(coor start_point, int& edge_prev_dir, int threshold, bool append){
	...
	if(guessed_corner){
		...
		if(i==5) {
			...
			else if (check_corner(left_edge[left_edge.size() - 11], left_edge[left_edge.size() - 1],
										 left_edge[0]))
				left_edge_corner.push_back(left_edge.size() - 11);
			else
...
void RightEdge(coor start_point, int& edge_prev_dir, int threshold, bool append){
	...
	if(guessed_corner){
		...
		if(i==10) {
			if (right_edge.size() > 16 && check_corner(right_edge[right_edge.size() - 11], right_edge[right_edge.size() - 1],
							 right_edge[right_edge.size() - 16]))
				right_edge_corner.push_back(right_edge.size() - 11);
			else if(check_corner(right_edge[right_edge.size() - 11], right_edge[right_edge.size() - 1],
					 right_edge[0]))
				right_edge_corner.push_back(right_edge.size() - 11);
			else
...
void normal_left_corner_fsm(Tstate& track_state,coor& final_point, coor& midpoint, coor left_start, coor right_start){
	...
	if(left_edge_corner[0]>10)
		...
		point1 = left_edge[0];
	coor new_start = {0, 0};
	if (jump(point1,left_edge[left_edge.size()-1],new_start,edge_threshold,false)){
...
void normal_right_corner_fsm(Tstate& track_state,coor& final_point, coor& midpoint, coor left_start, coor right_start){
	...
	if(right_edge_corner[0]>10)
		...
		point1 = right_edge[0];
	coor new_start = {0, 0};
	if (jump(point1,right_edge[right_edge.size()-1],new_start,edge_threshold,true)){
