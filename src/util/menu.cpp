...
Menu::Menu(St7735r *lcd, LcdConsole *console, Joystick *joystick, Flash *flash) {
	this->lcd = lcd;
...
void Menu::EnterMenu(Items *menu) {
	System::DelayMs(100);
	if (menu == &this->main_menu && joystick->GetState() == Joystick::State::kSelect) {
		Reset();
	}
...
void Menu::PrintItem(Item item, uint8_t row) {
	...
	if (row < max_row) {
		...
		if (focus == row) {
			if (item_selected) {
				console->SetBgColor(Lcd::kGreen);
			} else {
				console->SetBgColor(Lcd::kWhite);
			}
			console->SetTextColor(Lcd::kBlack);
		}
		console->WriteString("               ");
		console->SetCursorRow(row + 1);
...
void Menu::MenuAction(Items *menu) {
	...
	while (1) {
		...
		if (joystick_state == Joystick::State::kIdle) {
			System::DelayMs(100);
		} else if (joystick_state == Joystick::State::kSelect) {
			System::DelayMs(250);
		} else {
			last_trigger_time = System::Time() - last_trigger_time;
