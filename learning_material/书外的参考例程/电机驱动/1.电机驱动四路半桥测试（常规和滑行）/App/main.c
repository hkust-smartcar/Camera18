 ...
 *  @note       山外 电机 测试实验 ,山外 提供的 电机驱动模块，管脚都加入 MOS 管隔离，信号是反相的。
				...
				山外电机驱动，如果单片机输出的占空比为 20% ，那么真正输入给电机驱动就是 80% （反相了）
 */
void main()
{
    printf("\n*****FTM 电机测试 测试*****\n");
    ...
    FTM_PWM_init(MOTOR_FTM, MOTOR2_PWM,MOTOR_HZ,80);      //初始化 电机 PWM
    FTM_PWM_init(MOTOR_FTM, MOTOR3_PWM,MOTOR_HZ,60);      //初始化 电机 PWM
    FTM_PWM_init(MOTOR_FTM, MOTOR4_PWM,MOTOR_HZ,40);      //初始化 电机 PWM
